            ...
            <Field title="2. Project size">
              <div className="quote-options three">
                {sizeOptions.map(([key, label]) => (
                  <button type="button" className={form.size === key ? 'selected' : ''} onClick={() => update('size', key)} key={key}>{label}</button>
                ))}
              </div>
            </Field>
            <Field title="3. Complexity">
              <div className="quote-options three">
                {complexityOptions.map(([key, label]) => (
                  <button type="button" className={form.complexity === key ? 'selected' : ''} onClick={() => update('complexity', key)} key={key}>{label}</button>
                ))}
              </div>
            </Field>
            <Field title="4. Timeline">
              <div className="quote-options three">
                {timelineOptions.map(([key, label]) => (
                  <button type="button" className={form.timeline === key ? 'selected' : ''} onClick={() => update('timeline', key)} key={key}>{label}</button>
                ))}
              </div>
            </Field>
...
function Field({ title, children }) {
  return <div className="quote-field"><h3>{title}</h3>{children}</div>
}
