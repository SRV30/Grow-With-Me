  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.utils.toArray('.portfolio-card').forEach((card) => {
        const media = card.querySelector('.portfolio-media'), image = card.querySelector('.portfolio-image'), title = card.querySelector('.portfolio-title'), meta = card.querySelector('.portfolio-meta'), arrow = card.querySelector('.portfolio-view')
        const move = (event) => {
  ...
  return <div ref={root} className="portfolio-showcase">
    {projects.slice(0, 6).map((project, index) => <a href={`/work/${project.slug}`} key={project._id} className="portfolio-card" data-cursor="View project">
      <div className="portfolio-media">
        {project.coverImage?.url ? <CloudinaryImage src={project.coverImage.url} alt={project.coverImage.alt || project.title} className="portfolio-image" width={1600} sizes="(max-width: 768px) 100vw, 60vw" priority={index === 0} /> : <div className="portfolio-image portfolio-image-placeholder" />}
        <div className="portfolio-overlay" />
        <span className="portfolio-index">0{index + 1}</span><span className="portfolio-view"><ArrowUpRight size={18} /></span>
