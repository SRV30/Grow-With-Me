...
function getIntent(input) {
  ...
  if (/^(hi|hello|hey|namaste|good morning|good afternoon|good evening)/.test(value))
    return 'greeting'
...
const responseFor = (key) => {
  const responses = {
    ...
    website: {
      ...
      actions: [
        ['See Website Work', 'service-link:Website Design'],
        ['Start a Project', 'contact'],
      ],
    },
    social: {
      ...
      actions: [
        ['See Social Work', 'service-link:Social Media Management'],
        ['Start a Project', 'contact'],
      ],
    },
    reels: {
      ...
      actions: [
        ['See Reels Work', 'service-link:Reels & Video Editing'],
        ['Start a Project', 'contact'],
      ],
    },
    graphics: {
      ...
      actions: [
        ['See Design Work', 'service-link:Graphic Designing'],
        ['Start a Project', 'contact'],
      ],
    },
    ads: {
...
export default function ManualChatbot() {
        ...
        : [
            {
              id: 1,
              type: 'bot',
              text: 'Hi! I’m the Grow With Me assistant. What can I help you with?',
              actions: quickReplies,
            },
          ]
    } catch {
    ...
    getServices()
      .then((data) => {
        if (Array.isArray(data) && data.length)
          setServices(data.map((item) => item.title).filter(Boolean))
      })
      .catch(() => {})
  }, [])
  ...
  const addBot = (key) => {
    ...
    window.setTimeout(() => {
      ...
      setMessages((current) => [
        ...current,
        { id: `${Date.now()}-bot`, type: 'bot', text: response.text, actions: response.actions },
      ])
    }, 420)
  }
  ...
  const navigate = (key) => {
    ...
    else if (key.startsWith('service-link:'))
      window.location.href = `/work?service=${encodeURIComponent(key.split(':')[1])}`
    ...
    setMessages([
      {
        id: Date.now(),
        type: 'bot',
        text: 'Chat cleared. Hi again! What would you like help with?',
        actions: quickReplies,
      },
    ])
  }
      ...
      {open ? (
        <section className="gwm-chat-window" aria-label="Grow With Me chatbot">
          <header className="gwm-chat-header">
            <div className="gwm-chat-brand">
              ...
              <div>
                ...
                <span>Website assistant · Online</span>
              </div>
            </div>
            ...
            <>
              <div className="gwm-chat-context">
                ...
                {currentPage === 'portfolio'
                  ? 'You’re browsing our portfolio.'
                  : 'Ask me anything about Grow With Me.'}
              </div>
              ...
              <div className="gwm-chat-suggestions">
                {quickReplies.slice(0, 3).map(([label, key]) => (
          ...
          <div className="gwm-chat-footer">
            <Clock3 size={10} /> Manual assistant · No AI/API
          </div>
        </section>
      ) : null}
