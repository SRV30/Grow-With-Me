...
export default function ManualChatbot() {
  ...
  useEffect(() => {
    Promise.allSettled([getServices(), getProjects()]).then(([a, b]) => {
      if (a.status === 'fulfilled' && Array.isArray(a.value) && a.value.length) setServices(a.value)
      if (b.status === 'fulfilled' && Array.isArray(b.value)) setProjectCount(b.value.length)
    })
  }, [])
  useEffect(() => {
  ...
  const respond = (intent, raw = '') => {
    if (intent === 'greeting')
      return pushBot(
        'Hello! I can recommend a service, explain pricing, show work, explain our process, or collect your project requirements.',
        quickReplies,
      )
    ...
    if (intent === 'pricing')
      return pushBot(
        'Pricing depends on service, scope, deliverables, complexity and timeline. I can collect your requirements and send a proper enquiry.',
        [
          ['Get a quote', 'contact'],
    ...
    return pushBot(
      'I can help with services, recommendations, portfolio, pricing, process and project enquiries. Try “I need more customers from Instagram” or “I need a website”.',
      quickReplies,
    )
  }
