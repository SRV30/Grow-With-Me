...
function score(value, terms) {
  return terms.reduce(
    (sum, term) => sum + (value.includes(term) ? term.split(' ').length + 1 : 0),
    0,
  )
}
...
function findService(services, key) {
  ...
  return services.find((service) => {
    ...
    const slug = normalize(service.slug || slugify(service.title))
    return terms.some((term) => title.includes(term) || slug.includes(normalize(term)))
  })
}
function serviceDescription(title = '') {
  const value = normalize(title)
  if (value.includes('social media'))
    return 'Content planning, posting, captions, audience engagement and consistent brand presence.'
  if (value.includes('reel') || value.includes('video'))
    return 'Short-form videos, clean editing, transitions, subtitles and engaging presentation.'
  if (value.includes('graphic'))
    return 'Social posts, posters, offers, thumbnails and branded promotional creatives.'
  if (value.includes('advertising'))
    return 'Targeted paid campaigns designed to improve reach, leads, engagement and sales.'
  if (value.includes('promotion'))
    return 'Creative promotional content that helps products and services get noticed.'
  if (value.includes('website'))
    return 'Modern, responsive and professional websites for businesses and brands.'
...
function QuickEstimate({ services, onClose, onContact }) {
  const [form, setForm] = useState({
    service: services[0]?.title || 'Website Design',
    size: 'small',
    complexity: 'standard',
    timeline: 'flexible',
    addons: [],
    ...
    <div className="gwm-chat-estimate">
      <div className="gwm-chat-estimate-head">
        <div>
          ...
          <span>Choose options to get a starting range.</span>
        </div>
      ...
      <EstimateChoices
        title="Project size"
        options={estimateSizes}
        value={form.size}
        onChange={(value) => update('size', value)}
      />
      <EstimateChoices
        title="Complexity"
        options={estimateComplexity}
        value={form.complexity}
        onChange={(value) => update('complexity', value)}
      />
      <EstimateChoices
        title="Timeline"
        options={estimateTimeline}
        value={form.timeline}
        onChange={(value) => update('timeline', value)}
      />
      <div className="gwm-chat-estimate-label">Add-ons</div>
      <div className="gwm-chat-estimate-options gwm-chat-estimate-addons">
        {estimateAddons.map(([key, label]) => (
          <button
            type="button"
          ...
          >
            {form.addons.includes(key) ? <Check size={12} /> : null}
            {label}
          </button>
        ))}
      </div>
      <div className="gwm-chat-estimate-result">
        <span>Estimated range</span>
        <strong>
          {money(estimate.low)} — {money(estimate.high)}
        </strong>
        <small>Starting estimate only. Final price depends on exact requirements.</small>
      </div>
      <button
        type="button"
        className="gwm-chat-estimate-cta"
        onClick={() => onContact(form.service, money(estimate.low), money(estimate.high))}
      >
        Get this estimate <ChevronRight size={15} />
      </button>
    </div>
  )
}
function EstimateChoices({ title, options, value, onChange }) {
  return (
    <div>
      <div className="gwm-chat-estimate-label">{title}</div>
      <div className="gwm-chat-estimate-options">
...
function ProjectForm({ services, initialService, initialEstimate, onCancel }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setState('error')
      setError(
        requestError.response?.data?.message || 'Could not send the enquiry. Please try again.',
      )
    }
  }
  if (state === 'success')
    ...
    <form className="gwm-chat-project-form" onSubmit={submit}>
      <div className="gwm-chat-form-heading">
        <div>
          ...
          <span>Tell us what you want to achieve.</span>
        </div>
        <button type="button" onClick={onCancel} aria-label="Back to chat">
          <X size={16} />
        </button>
      </div>
      <input
...
export default function ManualChatbot() {
  ...
  const [showProjectForm, setShowProjectForm] = useState(false),
    [showEstimate, setShowEstimate] = useState(false),
    [formService, setFormService] = useState(''),
    [formEstimate, setFormEstimate] = useState('')
  const [services, setServices] = useState(fallbackServices.map((title) => ({ title }))),
    [projectCount, setProjectCount] = useState(null)
  const [profile, setProfile] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('gwm-chat-profile')) || {}
    } catch {
      return {}
    }
  })
        ...
        : [
            {
              id: 1,
              type: 'bot',
              text: 'Hi! I’m the Grow With Me assistant. Tell me your goal and I’ll help you choose the right service.',
  ...
  useEffect(() => {
    Promise.allSettled([getServices(), getProjects()]).then(([a, b]) => {
      const serviceList =
        a.status === 'fulfilled'
          ? Array.isArray(a.value)
            ? a.value
            : Array.isArray(a.value?.services)
              ? a.value.services
              : []
          : []
      const projectList =
        b.status === 'fulfilled'
          ? Array.isArray(b.value)
            ? b.value
            : Array.isArray(b.value?.projects)
              ? b.value.projects
              : []
          : []
      if (serviceList.length) setServices(serviceList)
      if (b.status === 'fulfilled') setProjectCount(projectList.length)
    })
  }, [])
  useEffect(() => {
    localStorage.setItem('gwm-chat-history', JSON.stringify(messages.slice(-60)))
    localStorage.setItem('gwm-chat-profile', JSON.stringify(profile))
    if (open && messagesRef.current)
      messagesRef.current.scrollTop = messagesRef.current.scrollHeight
  }, [messages, profile, open, typing])
  ...
  const pushBot = (text, actions = []) => {
    ...
    window.setTimeout(() => {
      ...
      setMessages((current) => [
        ...current,
        { id: `${Date.now()}-bot`, type: 'bot', text, actions },
      ])
    }, 380)
  }
  const showServices = () =>
    pushBot(
      `We currently have ${services.length} services. Pick one for details and related work.`,
      services.slice(0, 6).map((service) => [service.title, `service:${service.title}`]),
    )
  const recommend = (goal) => {
    const value = normalize(goal)
    let key = 'social'
    if (/website|web|online store/.test(value)) key = 'website'
    else if (/video|reel|youtube|short/.test(value)) key = 'reels'
    else if (/design|poster|thumbnail|creative/.test(value)) key = 'graphics'
    else if (/ads|advertising|leads|sales|paid/.test(value)) key = 'ads'
    else if (/promot|product|launch/.test(value)) key = 'promotion'
    const service = findService(services, key)
    const title =
      service?.title ||
      fallbackServices.find((item) => normalize(item).includes(key)) ||
      'Social Media Management'
    setProfile((current) => ({ ...current, goal, recommendedService: title }))
    pushBot(`For “${goal || 'your goal'}”, I’d start with ${title}. ${serviceDescription(title)}`, [
      ['See related work', `service:${title}`],
      ['Quick estimate', 'estimate'],
      ['Get a quote', 'contact'],
    ])
  }
  const respond = (intent, raw = '') => {
    if (intent === 'greeting')
      return pushBot(
        'Hello! I can recommend a service, give you a quick estimate, show work, explain our process, or collect your project requirements.',
        quickReplies,
      )
    if (intent === 'services') return showServices()
    if (intent === 'portfolio')
      return pushBot(
        `Our portfolio currently contains ${projectCount ?? 'published'} project${projectCount === 1 ? '' : 's'}. You can browse by service.`,
        [
    ...
    if (intent === 'pricing')
      return pushBot(
        'Want a fast starting number? Use the Quick Estimate, or I can collect your requirements for a proper enquiry.',
        [
          ['Quick estimate', 'estimate'],
          ['Get a quote', 'contact'],
        ],
    ...
    if (intent === 'process')
      return pushBot(
        'Our usual flow is Discuss → Plan → Create → Review → Publish → Grow. Your timeline depends on the project scope.',
        [
          ['Choose a service', 'services'],
          ['Quick estimate', 'estimate'],
          ['Start a project', 'contact'],
        ],
      )
    if (intent === 'recommend') return recommend(raw)
    if (['growth', 'brand'].includes(intent))
      return recommend(intent === 'growth' ? 'more customers and leads' : 'stronger branding')
    if (aliases[intent]) {
      const service = findService(services, intent)
      const title = service?.title || intent
      setProfile((current) => ({ ...current, lastService: title }))
      return pushBot(`${title}: ${serviceDescription(title)}`, [
        ['See related work', `service:${title}`],
        ['Quick estimate', 'estimate'],
        ['Get a quote', 'contact'],
      ])
    ...
    if (intent === 'contact') {
      setFormService(profile.recommendedService || profile.lastService || '')
      return setShowProjectForm(true)
    }
    return pushBot(
      'I can help with services, recommendations, portfolio, quick estimates, pricing, process and project enquiries. Try “I need a website” or “give me an estimate”.',
      quickReplies,
    )
  }
  const handleAction = (key, label) => {
    ...
    if (key === 'estimate') return respond('estimate')
    if (key === 'portfolio-link' || key.startsWith('service:')) {
      window.location.href =
        key === 'portfolio-link' ? '/work' : `/work?service=${encodeURIComponent(key.slice(8))}`
      return
    }
    respond(key, label)
  }
  const openEstimateContact = (service, low, high) => {
    setShowEstimate(false)
    setFormService(service)
    setFormEstimate(`Quick estimate: ${low} – ${high}`)
  ...
  const submit = (event) => {
    ...
    respond(intent, value)
  }
  const clearChat = () => {
    localStorage.removeItem('gwm-chat-history')
    localStorage.removeItem('gwm-chat-profile')
    setProfile({})
    setShowProjectForm(false)
    setShowEstimate(false)
    setMessages([
      {
        id: Date.now(),
        type: 'bot',
        text: 'New conversation started. What are you trying to achieve?',
        actions: quickReplies,
      },
    ])
  }
        ...
        <section className="gwm-chat-window" aria-label="Grow With Me chatbot">
          <header className="gwm-chat-header">
            ...
            <div className="gwm-chat-header-actions">
              <button type="button" onClick={clearChat} aria-label="Start new chat">
                <RotateCcw size={15} />
              </button>
          ...
          {showProjectForm ? (
            <div className="gwm-chat-project-wrap">
              <ProjectForm
                ...
                initialService={formService}
                initialEstimate={formEstimate}
                onCancel={() => {
                  setShowProjectForm(false)
                  setFormEstimate('')
                }}
            ...
            <>
              <div className="gwm-chat-context">
                ...
                {currentPage === 'portfolio'
                  ? 'Portfolio mode · I can help you find relevant work.'
                  : currentPage === 'project'
                    ? 'Project mode · Ask about services or start your own project.'
                    : profile.recommendedService
                      ? `Recommended: ${profile.recommendedService}`
                      : 'Tell me your goal and I’ll recommend a starting point.'}
              </div>
                ...
                {goals.map(([label, key]) => (
                  <button
                    ...
                    onClick={() => {
                      setMessages((current) => [
                        ...current,
                        { id: `${Date.now()}-user`, type: 'user', text: label },
                      ])
                      recommend(label)
                    }}
                  >
              ...
              <div className="gwm-chat-suggestions">
                {quickReplies.map(([label, key]) => (
                  <button type="button" key={key} onClick={() => handleAction(key, label)}>
                    {label}
                  </button>
                ))}
              </div>
              <form className="gwm-chat-input" onSubmit={submit}>
                <input
                  ...
                  placeholder="Tell me your goal or ask a question..."
                  aria-label="Type your question"
                  maxLength={500}
                />
                <button type="submit" disabled={typing} aria-label="Send message">
                  <Send size={17} />
                </button>
              </form>
            </>
          )}
          <div className="gwm-chat-footer">
            <Clock3 size={10} /> Manual smart assistant · No AI/API
          </div>
        </section>
      ) : null}
      <button
