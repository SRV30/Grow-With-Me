...
function score(value, terms) {
  return terms.reduce(
    (total, term) => total + (value.includes(term) ? term.split(' ').length + 1 : 0),
    0,
  )
}
...
function describeService(title = '') {
  ...
  if (value.includes('social media management'))
    return 'Content planning, posting, captions, audience engagement and a consistent brand presence.'
  if (value.includes('reels') || value.includes('video'))
    return 'Short-form videos, clean editing, transitions, subtitles and engaging presentation.'
  if (value.includes('graphic'))
    return 'Posts, posters, offers, thumbnails and branded promotional creatives.'
    ...
    <form className="gwm-chat-project-form" onSubmit={submit}>
      <div className="gwm-chat-form-heading">
        <div>
          ...
          <span>Get a tailored conversation, not a generic form.</span>
        </div>
...
export default function ManualChatbot() {
        ...
        : [
            {
              id: 1,
              type: 'bot',
              text: 'Hi! I’m the Grow With Me assistant. Tell me what you want to achieve and I’ll guide you.',
  ...
  useEffect(() => {
    Promise.allSettled([getServices(), getProjects()]).then(([serviceResult, projectResult]) => {
      if (
        ...
        Array.isArray(serviceResult.value) &&
        serviceResult.value.length
      )
  ...
  const showServices = () =>
    pushBot(
      `We currently offer ${services.length} services. Pick one and I’ll explain what it includes and show the relevant work.`,
  ...
  const recommend = (goal = '') => {
    ...
    const title =
      service?.title ||
      fallbackServices.find((item) => normalize(item).includes(intent)) ||
      'Social Media Management'
    pushBot(`Based on that goal, I’d start with ${title}. ${describeService(title)}`, [
      ['See this service', `service:${title}`],
  ...
  const respond = (intent, raw = '') => {
    if (intent === 'greeting')
      return pushBot(
        'Hello! I can recommend a service, explain pricing, show portfolio work, explain the process, or collect your project requirements.',
        quickReplies,
      )
    if (intent === 'services') return showServices()
    if (intent === 'portfolio')
      return pushBot(
        `Our portfolio has ${projectCount ?? 'selected'} published projects. Open it to browse projects by service.`,
        [
    ...
    if (intent === 'pricing')
      return pushBot(
        'There is no one fixed price. Pricing depends on service, scope, content, timeline and requirements. I can collect your details now so the enquiry is specific.',
        [
          ['Get a quote', 'contact'],
    ...
    if (intent === 'process')
      return pushBot('Our usual process is: Discuss → Plan → Create → Review → Publish → Grow.', [
    ...
    if (['social', 'reels', 'graphics', 'ads', 'promotion', 'website'].includes(intent)) {
      ...
      return pushBot(`${title}: ${describeService(title)}`, [
    ...
    return pushBot(
      'I can help with services, portfolio, pricing, process and project requirements. Try: “I want more customers from Instagram” or “I need a website”.',
      quickReplies,
    )
  }
    ...
    setMessages([
      {
        id: Date.now(),
        type: 'bot',
        text: 'New conversation started. What would you like to achieve?',
        actions: quickReplies,
      },
    ])
  }
  const contextText =
    currentPage === 'portfolio'
      ? 'Portfolio mode · I can help you find the right service.'
      : currentPage === 'project'
        ? 'Project mode · Ask about this work or start your own project.'
        : 'Online · Ask about services, pricing or your project.'
      ...
      {open ? (
        <section className="gwm-chat-window" aria-label="Grow With Me chatbot">
          <header className="gwm-chat-header">
            <div className="gwm-chat-brand">
              ...
              <div>
                ...
                <span>Smart website assistant</span>
              </div>
            </div>
            ...
            <>
                ...
                {goals.map((goal) => (
                  <button
                    ...
                    onClick={() => {
                      setMessages((current) => [
                        ...current,
                        { id: `${Date.now()}-user`, type: 'user', text: goal },
                      ])
                      recommend(goal)
                    }}
                  >
              ...
              <form className="gwm-chat-input" onSubmit={submit}>
                <input
                  ...
                  placeholder="Try: I need more customers..."
          ...
          <div className="gwm-chat-footer">
            ...
            <span>
              <CircleHelp size={10} /> Private browser chat
            </span>
          </div>
        </section>
      ) : null}
