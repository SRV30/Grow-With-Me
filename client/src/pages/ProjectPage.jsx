  ...
  return (
    <main ref={root} className="project-detail-page">
      ...
      <section className="project-detail-container project-detail-intro" data-project-reveal>
        ...
        <div>
          <p className="project-detail-label">The brief</p>
          <blockquote>Good creative starts with a clear purpose.</blockquote>
        </div>
        <p>
          {project.description ||
            'This case study presents the available project brief, services and final creative work.'}
        </p>
      </section>

      <section className="project-detail-container project-detail-case-study" data-project-reveal>
        <div className="project-detail-case-card">
          <span>01</span>
          <div>
            <p className="project-detail-label">Challenge</p>
            <h2>Understand the goal first.</h2>
            <p>
              {project.description ||
                'The project was shaped around the client, category and intended audience.'}
            </p>
          </div>
        </div>
        <div className="project-detail-case-card">
          <span>02</span>
          <div>
            <p className="project-detail-label">Approach</p>
            <h2>Turn the brief into focused creative.</h2>
            <p>
              We brought the selected services together into one consistent direction, keeping the
              final experience clear and purposeful.
            </p>
          </div>
        </div>
        <div className="project-detail-case-card">
          ...
          <div>
            ...
            <h2>Build the final deliverables.</h2>
            <p>
              {services.length
                ? `The work covered ${services.map(formatCategory).join(', ')}.`
                : `The final work was delivered as a ${category.toLowerCase()} project.`}
            </p>
          </div>
        </div>
      </section>
        ...
        <section className="project-detail-container project-detail-focus" data-project-reveal>
          {services.map((service) => (
            <div className="project-detail-focus-item" key={service}>
              ...
              <div>
                <strong>{formatCategory(service)}</strong>
                <span>Part of the project deliverables and creative direction.</span>
              </div>
            </div>
          ))}
        </section>
      ...
      {mediaCount ? (
        <section
          className="project-detail-container project-detail-gallery-section"
          data-project-reveal
        >
          <div className="project-detail-section-heading">
            <div>
              <p className="project-detail-label">04 · Final work</p>
              <h2>Inside the project.</h2>
            </div>
            <p>
              {mediaCount} visual asset{mediaCount === 1 ? '' : 's'}
            </p>
          </div>
            ...
            {visibleGallery.map((item, index) => (
              ...
              >
                <CloudinaryImage
                  ...
                  width={1400}
                  sizes="(max-width: 620px) 100vw, 70vw"
                />
              </figure>
            ))}
            {videos.map((item, index) => (
              <figure className="project-detail-media" key={`video-${item.publicId || item.url}`}>
                <video
                  controls
                  preload="metadata"
                  poster={item.thumbnail || undefined}
                  src={item.url}
                  aria-label={`${project.title} project video ${index + 1}`}
                />
              </figure>
            ))}
          </div>
      ...
      {related.length ? (
        <section className="project-detail-container project-detail-related" data-project-reveal>
          <div className="project-detail-section-heading">
            <div>
              <p className="project-detail-label">More from {category}</p>
              <h2>Related work.</h2>
            </div>
            <Link
              to={`/work?service=${encodeURIComponent(project.category)}`}
      ...
      <section className="project-detail-cta">
        <div className="project-detail-container project-detail-cta-grid">
          <div data-project-reveal>
            <p className="project-detail-cta-kicker">05 · Your project</p>
            <h2>Let&apos;s create work worth showing.</h2>
            <p>Have a brief? Let&apos;s turn it into something people remember.</p>
          </div>
