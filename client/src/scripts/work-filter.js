...
const syncServices = async () => {
  ...
  try {
    ...
    const serviceList = Array.isArray(payload.data) ? shuffle(payload.data) : []
    ...
    serviceList.forEach((service, index) => {
      ...
      card.href = `/services/${encodeURIComponent(service.slug || slugify(service.title))}`
      card.setAttribute('aria-label', `View ${service.title} service`)
      card.hidden = false
    })
    serviceCards.slice(serviceList.length).forEach((card) => card.remove())
    ...
    if (contactSelect && contactSelect.dataset.gwmServicesSynced !== 'true') {
      ...
      contactSelect.value = serviceList.some((service) => service.title === current) ? current : ''
...
const syncProjectCategorySelect = async () => {
  ...
  try {
    ...
    if (!response.ok) return
    const payload = await response.json()
    const serviceList = Array.isArray(payload.data) ? payload.data : []
    if (!serviceList.length) return
    ...
    if (serviceList.some((service) => service.slug === current)) select.value = current
  ...
  if (!document.documentElement.dataset.gwmWorkFilterBound) {
    document.addEventListener('click', (event) => {
      ...
      if (serviceCard) {
        ...
        window.location.href = `/services/${encodeURIComponent(slug || slugify(title || ''))}`
      }
    })
