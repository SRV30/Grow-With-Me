...
export default function UserManagement() {
  ...
  const createUser = async (event) => {
    ...
    try {
      ...
      setMessage(
        `User ${user.name} created successfully. They can sign in with the configured default password.`,
      )
      await loadUsers()
    } catch (e) {
  ...
  const removeUser = async (user) => {
    if (!window.confirm(`Delete admin account for ${user.name}? This action cannot be undone.`))
      return
      ...
      <div className="admin-user-management-grid">
        <section className="admin-panel admin-user-panel">
          <div className="admin-panel-title">
            <div>
              <h2>
                <UserPlus size={19} /> Create new user
              </h2>
