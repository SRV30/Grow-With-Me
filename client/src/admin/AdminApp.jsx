...
import { FolderKanban, Image, LogOut, Plus, RefreshCw, ShieldCheck, Star, Trash2 } from 'lucide-react'
import { deleteAdminProject, getAdminProjects, getCurrentAdmin, loginAdmin, logoutAdmin, updateProjectFlags } from './api.js'
...
function Login({ onLogin }) {
  ...
  const submit = async (event) => { event.preventDefault(); setError(''); setLoading(true); try { await loginAdmin({ email, password }); onLogin() } catch (e) { setError(e.response?.data?.message || 'Unable to sign in') } finally { setLoading(false) } }
...
function Dashboard({ admin, onLogout }) {
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><div><div className="admin-brand">GROW<span>.</span>WITH<span>.</span>ME</div><p className="admin-side-label">CMS</p><button className={view === 'projects' ? 'admin-nav-item active' : 'admin-nav-item'} onClick={() => setView('projects')}><FolderKanban size={17} /> Projects</button><button className={view === 'media' ? 'admin-nav-item active' : 'admin-nav-item'} onClick={() => setView('media')}><Image size={17} /> Media Library</button></div><div><div className="admin-user"><div className="admin-avatar">{admin.name?.[0] || 'A'}</div><div><strong>{admin.name}</strong><small>{admin.email}</small></div></div><button className="admin-logout" onClick={onLogout}><LogOut size={16} /> Sign out</button><a className="admin-back-side" href="/">View website ↗</a></div></aside><main className="admin-content">{view === 'media' ? <MediaLibrary /> : <><header className="admin-topbar"><div><p className="admin-eyebrow">Content management</p><h1>Projects</h1></div><div className="admin-actions"><button className="admin-icon" onClick={load} title="Refresh"><RefreshCw size={17} /></button><button className="admin-primary admin-add" onClick={() => setEditor('new')}><Plus size={17} /> New project</button></div></header>{error && <div className="admin-alert">{error}</div>}<section className="admin-stats"><div><span>Total projects</span><strong>{projects.length}</strong></div><div><span>Published</span><strong>{projects.filter(p => p.published).length}</strong></div><div><span>Featured</span><strong>{projects.filter(p => p.featured).length}</strong></div></section><section className="admin-table-wrap"><div className="admin-table-head"><span>Project</span><span>Category</span><span>Status</span><span>Actions</span></div>{loading ? <div className="admin-empty">Loading projects…</div> : projects.length === 0 ? <div className="admin-empty">No projects yet. Create your first project.</div> : projects.map(project => <article className="admin-project-row" key={project._id}><button className="admin-project-link" onClick={() => setEditor(project)}><strong>{project.title}</strong><small>{project.client || 'No client'} · {project.year || '—'}</small></button><span className="admin-category">{project.category.replace('-', ' ')}</span><div className="admin-statuses"><button className={project.published ? 'status on' : 'status'} onClick={() => toggle(project._id, 'published', !project.published)}>{project.published ? 'Published' : 'Draft'}</button><button className={project.featured ? 'status star on' : 'status star'} title="Toggle featured" onClick={() => toggle(project._id, 'featured', !project.featured)}><Star size={14} fill={project.featured ? 'currentColor' : 'none'} /></button></div><button className="admin-danger" onClick={() => remove(project._id)} title="Delete"><Trash2 size={16} /></button></article>)}</section></>}</main></div>
}
